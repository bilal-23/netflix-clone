...
const ShowDetails = ({ mediaType }) => {
  ...
  const detailMovieUrl = `${mediaType}/${movie_id}?api_key=${API_KEY}`;
  const castAndCrewUrl = `${mediaType}/${movie_id}/credits?api_key=${API_KEY}&language=en-US`;
  const externalIdUrl = `${mediaType}/${movie_id}/external_ids?api_key=${API_KEY}&language=en-US`;
  const similarUrl = `${mediaType}/${movie_id}/similar?api_key=${API_KEY}&language=en-US`;
  ...
  useEffect(() => {
    ...
    axios
      .get(detailMovieUrl)
      .then((req) => setMovie(req.data))
      .catch((err) => setError(err));
    axios
      .get(castAndCrewUrl)
      .then((req) => setCastCrew(req.data))
      .catch((err) => setError(err));
    axios
      .get(externalIdUrl)
      .then((req) => setExternalLinks(req.data))
      .catch((err) => setError(err));
    axios
      .get(similarUrl)
      .then((req) => setSimilarMovies(req.data))
      .catch((err) => setError(err));
    ...
    <>
      ...
      <div className="movie__details">
        <div className="detail__content">
          <div
            ...
            style={{
              ...
              transition: "all 1s",
              visibility: imageIsLoaded && "visible",
            }}
          >
            ...
            <div className="detail__externalLinks">
              {externalLinks?.instagram_id && (
                <a
                  ...
                  className="detail__externalLink"
                >
                  <svg
                    stroke="currentColor"
                    fill="none"
            ...
            <div
              ...
              style={{ opacity: castImageLoaded ? "1" : "0" }}
            >
          ...
          <div
            className="detail__content__poster"
        ...
        {similarMoviesArray.length > 0 && (
          <SimilarMovies
            similarMovies={similarMoviesArray}
            mediaType={mediaType}
